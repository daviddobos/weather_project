...
combined_forecast_df = all_forecast_dfs[0]
for df in all_forecast_dfs[1:]:
    combined_forecast_df = combined_forecast_df.union(df)

deduplicated_combined_forecast_df = combined_forecast_df.dropDuplicates()
...
distinct_values_df = deduplicated_combined_forecast_df.select(col('country')).distinct()
...
translated_final_df = deduplicated_combined_forecast_df.join(translated_spark_df, on='country', how="left")
